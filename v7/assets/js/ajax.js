...
$(document).on('change keyup blur','.changesNo',function(){
	...
	alert(id_arr);
	id = id_arr.split("_");
	quantity = $('.quy'+id[1]).val();
	price = $('.pri'+id[1]).val();
	if( quantity!='' && price !='' ) $('.to'+id[1]).val( (parseFloat(price)*parseFloat(quantity)).toFixed(2) );	
	calculateTotal();
});
...
function calculateTotal(){
	...
	$('.totalLinePrice').each(function(){
		if($(this).val() != '' )to += parseFloat( $(this).val() );
	});
	$('#to').val( to.toFixed(2));
	//alert('gdfgd');
	tax = $('#tax').val();
