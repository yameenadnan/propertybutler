...
class Bms_task extends CI_Controller {
    ...
    public function task_list() {
		...
		$data['browser_title'] = 'Property Butler | Minor Task List';
        ...
        if($_SESSION['bms']['user_type'] == 'jmb') {
            ...
            $data['property_id'] = isset($_GET['property_id']) ? trim ($_GET['property_id']) : '';            
    ...
    function get_task_list () {
        ...
        if (isset($_POST['offset']) && is_numeric($_POST['offset']) && $_POST['offset'] >= 0 && isset($_POST['rows']) && is_numeric($_POST['rows']) && $_POST['rows'] >= 0 && $_POST['rows'] <= 100) {
            ...
            $task_update = !empty($_POST['task_update']) ? trim ($_POST['task_update']) : '';
            $tasks = $this->bms_task_model->get_task ('own',$_SESSION['bms']['designation_id'],$_SESSION['bms']['staff_id'],$_POST['offset'],$_POST['rows'],$property_id,$task_status,$task_sear_id,$task_search_txt,$sort_by,$task_update);                
        }
    ...
    function get_os_task_list () {
        ...
        if (isset($_POST['offset']) && is_numeric($_POST['offset']) && $_POST['offset'] >= 0 && isset($_POST['rows']) && is_numeric($_POST['rows']) && $_POST['rows'] >= 0 && $_POST['rows'] <= 100) {
            ...
            $task_update = !empty($_POST['task_update']) ? trim ($_POST['task_update']) : '';
            if($_SESSION['bms']['user_type'] == 'jmb') {
                $os_tasks = $this->bms_task_model->get_task_with_num_rows_jmb ($property_id,$task_status,$_POST['offset'],$_POST['rows'],$task_sear_id,$task_search_txt,$sort_by,'',$task_update);
            } else {
                $os_tasks = $this->bms_task_model->get_task_with_num_rows ('oversee',$_SESSION['bms']['designation_id'],$_SESSION['bms']['staff_id'],$_POST['offset'],$_POST['rows'],$property_id,$task_status,$task_sear_id,$task_search_txt,$sort_by,$task_update);
            }                
        }
    ...
    function new_task_submit () {
       // echo "<pre>";print_r($_POST);echo "</pre>"; echo "<pre>";print_r($_FILES);echo "</pre>";exit;
        if(isset($_POST) && !empty($_POST['task'])) {
            ...
            if (!empty($_POST['resident_email_hidd']) & (filter_var($_POST['resident_email_hidd'], FILTER_VALIDATE_EMAIL)) && $_POST['resident_valid_email'] == 1) {
                ...
                $this->bms_masters_model->update_unit_set_invalid_email ($data_unit, $task['unit_id']);
            }
        }
