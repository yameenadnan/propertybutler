              ...
              <div class="box-body">
                  <div class="row">
                        ...
                        <select class="form-control" id="service_id" name="service_id">
                            ...
                            <?php 
                                foreach ($services as $key=>$val) {
                                    $selected = !empty($_GET['service_id']) && trim($_GET['service_id']) == $val['service_id'] ? 'selected="selected" ' : '';
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Question" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_sfs/sfs_question_form');?>';
        return false;
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_sfs/get_question_list');?>',
        data: {'offset':offset,'rows':rows,'service_id':$('#service_id').val(),'search_txt':search_txt},
        ...
        success: function(data) {  
            ...
            if ( numFound > 0 ) {
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.question_name+'</td>';
                    str += '<td>'+item.remarks+'</td>';
                    str += '<td>'+item.amount+'</td>';
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_sfs/sfs_question_form');?>/'+item.question_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_sfs/sfs_question_list');?>/'+offset+'/'+rows+'?service_id='+$('#service_id').val()+'&search_txt='+search_txt;
