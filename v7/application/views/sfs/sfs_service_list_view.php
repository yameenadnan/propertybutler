              ...
              <div class="box-body">
                  <div class="row">
                    <div class="col-md-4 col-xs-6">
                        <select class="form-control" id="cat_id" name="cat_id">
                            ...
                            foreach ($categories as $key=>$val) {
                                $selected = !empty($cat_id) && trim($cat_id) == $val['cat_id'] ? 'selected="selected" ' : '';
                    ...
                    <div class="col-md-4 col-xs-1 no-padding">
                        ...
                        <input class="btn btn-primary" style="margin-left: 3px;" onclick="window.location.href='<?php echo base_url('index.php/bms_sfs/sfs_service_form');?>'" value="Add Service" type="button">
                    </div>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_sfs/get_sfs_service_list');?>',
        data: {'offset':offset,'rows':rows,'cat_id':$('#cat_id').val(),'search_txt':search_txt},
        ...
        success: function(data) {
            ...
            if(numFound > 0) {
                ...
                $.each(data.records,function (i, item) {
                    ...
                    str += '<td>'+ ( (item.commis_amount != null) ? item.commis_amount:'-') +'</td>';
                    str += '<td>'+ ( (item.commis_percent != null) ? item.commis_percent:'-') +'</td>';
                    str += '<td>'+item.no_of_question+'</td>';
                    str += '<td>'+ ( (item.quote_type == '1') ? 'Open':'Fixed') +'</td>';
                    str += '<td style="text-align: center;">';
                    str += '<a href="<?php echo base_url('index.php/bms_sfs/sfs_service_form');?>/'+item.service_id+'" title="Edit"><i class="fa fa-edit"></i></a>';
                    str += '</td>';
                    str += '</tr>';
                });
            ...
            if (flag) {
                if (typeof (history.pushState) != "undefined") {
                    offset_link = offset;
                    rows_link = rows;
                    var update_url = '<?php echo base_url('index.php/bms_sfs/sfs_service_list');?>/'+offset+'/'+rows+'?cat_id='+$('#cat_id').val()+'&search_txt='+search_txt;
