              ...
              <div class="box-body">
                  <div class="row">
                        ...
                        <select class="form-control" id="state_id" name="state_id">
                            ...
                            <?php 
                                foreach ($states as $key=>$val) { 
                                    $selected = isset($_GET['state_id']) && trim($_GET['state_id']) != '' && trim($_GET['state_id']) == $val['state_id'] ? 'selected="selected" ' : '';  
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add TSP" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_sfs/tsp_form');?>';
        return false;
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_sfs/get_tsp_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.tsp_name+'</td>';
                    str += '<td>'+item.company_reg_no+'</td>';
                    str += '<td>'+item.company_phone_1+'</td>';
                    ...
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_sfs/tsp_form');?>/'+item.tsp_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_sfs/tsp_list');?>/'+offset+'/'+rows+'?state_id='+$('#state_id').val()+'&search_txt='+search_txt;
