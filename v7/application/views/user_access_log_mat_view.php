            ...
            <!-- /.box-header -->
              <div class="box-body">
                  <div class="row">
                    <div class="col-md-4 col-xs-5">
                        <div class="form-group">
                            ...
                            <div class="input-group date">
                              ...
                              <input class="form-control pull-right" name="mat_date" id="datepicker" type="text"  value="<?php echo isset($_GET['mat_date']) && trim($_GET['mat_date']) != '' ? date('d-m-Y',strtotime($_GET['mat_date'])) : '' ;?>" />
                            </div>
                            <!-- /.input group -->
                          </div>
                    </div>
                    ...
                    <div class="col-md-4 col-xs-2" style="margin-top:25px">
...
<div id="myModal23" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Staff Accessed IP Details</h4>
      </div>
      <div class="modal-body modal-body23">
...
$(document).ready(function () {
    ...
    $('.log_filter').click(function () {
        window.location.href="<?php echo base_url('index.php/bms_user_access_log/user_access_log_matching');?>?mat_date="+$('#datepicker').val();
        return false;  
        loadLogList(0,100);      
    });
...
function loadLogList (offset,rows) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_user_access_log/get_user_access_log_mat');?>',
        data: {'mat_date':$('#datepicker').val(),'offset':offset,'rows':rows},
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    var d = new Date(item.a_date);
                    str += '<tr>';
                    str += '<td class="hidden-xs">'+(eval(offset)+eval(i)+1)+'</td>';
                    str += '<td><a href="javascript:;" class="staff_property" data-value="'+item.staff_id+'">'+item.first_name+' '+item.last_name+ '</a> (<a href="<?php echo base_url('index.php/bms_user_access_log/user_access_log_list');?>?desi_id='+item.designation_id+'&staff_id='+item.staff_id+'&rec_date='+$('#datepicker').val()+'&group=1" target="_blank" class="staff_property" data-value="'+item.staff_id+'">' + item.hit_cnt +'</a>)</td>';
                    //str += '<td><a href="<?php echo base_url('index.php/bms_user_access_log/user_access_log_list');?>?desi_id='+item.designation_id+'&staff_id='+item.staff_id+'&rec_date='+$('#datepicker').val()+'&group=1" target="_blank" class="staff_property" data-value="'+item.staff_id+'">'+item.first_name+' '+item.last_name+ ' (' + item.hit_cnt +')</a></td>';                    
                    ...
                    str += '<td><a href="javascript:;" class="staff_ip_addr" data-value="'+item.staff_id+'">'+item.ip_address+'</a></td>';
                ...
                $('.staff_ip_addr').unbind('click');
                $('.staff_ip_addr').bind("click",function () {
                    $('.modal-body23').load('<?php echo base_url('index.php/bms_user_access_log/staff_ip_details/');?>'+$(this).attr('data-value')+'?mat_date='+$('#datepicker').val(),function(result){
