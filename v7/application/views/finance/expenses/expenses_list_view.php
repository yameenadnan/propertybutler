        ...
        success: function(data) {
            ...
            if(numFound > 0) {
                $.each(data.records,function (i, item) {
                    ...
                    str += '<td>';
                                var paystatus = "";
                                if ( item.inv_paid_status == '0' ) {
                                    if(item.paid_amount!='0.00' && item.paid_amount!=item.total) {
                                        str += '<span class="label label-warning large" style="padding: 8px;">PARTLY PAID</span>';
                                    } else if (item.paid_amount !='0.00' && item.paid_amount ==item.total) {
                                        str += '<span class="label label-warning large" style="padding: 8px;">PARTLY PAID</span>';
                                    } else if (item.paid_amount=='0.00') {
                    ...
                    if ( item.inv_paid_status == '0' ) {
                        if ( item.paid_amount != '0.00' && item.paid_amount != item.total ) {
                            if ('<?php echo in_array($_SESSION['bms']['designation_id'], $this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1') {
                                str += '<a href="<?php echo base_url('index.php/bms_fin_expenses/add_expenses');?>/' + item.exp_inv_id + '/?invoice_paid=yes' + '" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                    ...
                    } else if ( item.inv_paid_status == '1' ) {
                        if ('<?php echo in_array($_SESSION['bms']['designation_id'], $this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1') {
                            str += '<a href="<?php echo base_url('index.php/bms_fin_expenses/add_expenses');?>/' + item.exp_inv_id + '/?invoice_paid=yes' + '" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                            str += '<a href="javascript:;" data-id="' + item.exp_inv_id + '" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                        }
                ...
                str += '<td class="visible-xs text-center" colspan="10">No Record Found</td></tr>';
                $("#content_tbody").html(str);
