              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-2 col-xs-12" style="padding-top: 25px;">
                        <input class="btn btn-primary add_btn" value="New Deposit" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_fin_dp_receive/add_dp_receive');?>?property_id='+$("#property_id").val();
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_fin_dp_receive/get_dp_receive_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td><a href="<?php echo base_url().'index.php/bms_fin_dp_receive/dp_receive_details/';?>'+item.depo_receive_id+'">'+item.doc_ref_no+'</a></td>';
                    str += '<td>'+payment_mode[item.payment_mode]+'</td>';
                    str += '<td>'+item.unit_no+'</td>';                    
                    str += '<td>'+item.owner_name+'</td>';
                    str += '<td>'+(item.deposit_date != '' ? formatDate(item.deposit_date) : '')+'</td>';
                    //str += '<td>'+(item.bill_due_date != '' ? formatDate(item.bill_due_date) : '')+'</td>';
                    str += '<td>'+item.amount+'</td>';  
                                  
                    str += '<td style="text-align: left;">';
                    str += '<a href="javascript:;" onclick="printDiv(\'<?php echo base_url('index.php/bms_fin_dp_receive/dp_receive_details');?>/'+item.depo_receive_id+'/download\')" title="Download"><i class="fa fa-download"></i></a>&ensp;';
                    str += '<a href="javascript:;" onclick="printDiv(\'<?php echo base_url('index.php/bms_fin_dp_receive/dp_receive_details');?>/'+item.depo_receive_id+'/print\')" title="Print"><i class="fa fa-print"></i></a>&ensp;';
                    if(parseFloat(item.open_credit) > 0) {
                        str += '<a href="<?php echo base_url('index.php/bms_fin_dp_receive/add_dp_receive');?>/'+item.depo_receive_id+'/amend" title="Amend"><i class="fa fa-pencil"></i></a>&ensp;';
                    }
                    
                    if('<?php echo in_array($_SESSION['bms']['designation_id'],$this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1' && item.refund_status == '0')  { 
                        //str += '<a href="<?php echo base_url('index.php/bms_fin_dp_receive/add_dp_receive');?>/'+item.bill_id+'" title="Edit"><i class="fa fa-info"></i></a>&ensp;';
                        str += '<a href="<?php echo base_url('index.php/bms_fin_dp_receive/add_dp_receive');?>/'+item.depo_receive_id+'" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                        str += '<a href="javascript:;" data-id="'+item.depo_receive_id+'" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                    }
                    str += '</td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_fin_dp_receive/dp_receive_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&unit_id='+$('#unit_id').val();
...
function remove_item (id){
    ...
    if(confirm('You cannot undo this action. Are you sure want to delete?')) {
        $.ajax({
            type:"post",
            async: true,
            url: '<?php echo base_url('index.php/bms_fin_dp_receive/unset_dp_receive');?>',
            data: {'depo_receive_id':id},
