              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-4 col-xs-5">                        
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search Bank">
                    </div>
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Bank" type="button">
                    </div>
...
var acc_type = $.parseJSON('<?php echo json_encode($this->config->item('bank_acc_type'));?>');
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_fin_banks/add_bank');?>?property_id='+$("#property_id").val();
        return false;  
    });  
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_fin_banks/get_banks_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.bank_name+'</td>';
                    str += '<td>'+(typeof(item.acc_type) != 'undefined' && item.acc_type != '0' ? acc_type[item.acc_type] : '-')+'</td>';                    
                    str += '<td>'+item.acc_no+'</td>';
                    str += '<td>'+item.opening_bal+'</td>';
                    console.log(item.ob_date);
                    str += '<td>'+(typeof(item.ob_date) != 'undefined' ? formatDate(item.ob_date) : '')+'</td>';
                      
                    str += '<td>'+(item.acc_status == '0' ? 'Deactive' : 'Active')+'</td>';                  
                    str += '<td style="text-align: center;">';
                    
                    str += '<a href="<?php echo base_url('index.php/bms_fin_banks/add_bank');?>/'+item.bank_id+'" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                    //str += '<a href="javascript:;" data-id="'+item.bank_id+'" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_fin_banks/bank_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;
...
function remove_item (id){
    ...
    if(confirm('You cannot undo this action. Are you sure want to delete?')) {
        $.ajax({
            type:"post",
            async: true,
            url: '<?php echo base_url('index.php/bms_fin_banks/unset_bank');?>',
            data: {'bank_id':id},
