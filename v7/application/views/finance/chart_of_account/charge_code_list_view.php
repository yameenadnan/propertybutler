              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_asset_btn" value="Add Chart Of Accounts" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_asset_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_charge_codes/charge_code_form');?>';
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_charge_codes/get_charge_code_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+(group_name != item.charge_code_group_name ? item.charge_code_group_name : '')+'</td>';
                    str += '<td>'+(cat_name != item.charge_code_category_name ? item.charge_code_category_name : '')+'</td>';
                    str += '<td>'+(item.charge_code_sub_category_name == null ? '' : item.charge_code_sub_category_name) +'</td>';
                                    
                    str += '<td style="text-align: center;">';
                    //str += '<a href="javascript:;" class="asset_det_cls" data-value="'+item.asset_id+'" title="View"><i class="fa fa-info-circle"></i></a>&ensp;';
                    if(cat_name != item.charge_code_category_name) {
                        str += '<a href="<?php echo base_url('index.php/bms_charge_codes/charge_code_form');?>/'+item.charge_code_category_id+'" title="Edit"><i class="fa fa-edit"></i></a>';
                    }
                    
                    str += '</td>';
                    str += '</tr>';
                    if(group_name != item.charge_code_group_name) {
                        group_name = item.charge_code_group_name;
                    }
                    if(cat_name != item.charge_code_category_name) {
                        cat_name = item.charge_code_category_name;
                    }
                });
                ...
                $('.asset_det_cls').unbind('click');
                $('.asset_det_cls').bind("click",function () {
                    $('.modal-content').css('width','750px');
                    $('.modal-body2').load('<?php echo base_url('index.php/bms_charge_codes/asset_details/');?>'+$(this).attr('data-value'),function(result){
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_charge_codes/charge_codes_list');?>/'+offset+'/'+rows+'?search_txt='+search_txt;
