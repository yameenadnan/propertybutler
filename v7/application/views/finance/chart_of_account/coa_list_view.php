            ...
            ?>
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add COA" type="button">
                    </div>
              ...
              <table id="example2" class="table table-bordered table-hover table-striped">
                ...
                <tbody id="content_tbody">
                    <tr><td class="hidden-xs text-center" colspan="7">Loading...</td>
                    <td class="visible-xs text-center" colspan="4">Loading...</td></tr>           
                </tbody>                
              </table>
            </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_fin_coa/coa_form');?>';
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    if($('#property_id').val() != '') {
        ...
        $.ajax({
            type:"post",
            async: true,
            url: '<?php echo base_url('index.php/bms_fin_coa/get_coa_list');?>',
            ...
            success: function(data) {  
                ...
                if(numFound > 0) {                    
                    $.each(data.records,function (i, item) { 
                        ...
                        str += '<td>'+item.coa_code+'</td>';
                        str += '<td>'+item.coa_name+'</td>';
                        str += '<td>'+item.coa_type_name+'</td>';
                                        
                        str += '<td style="text-align: center;">';
                        str += '<a href="<?php echo base_url('index.php/bms_fin_coa/coa_form');?>/'+item.coa_id+'" title="Edit"><i class="fa fa-edit"></i></a>';
                        str += '</td>';
                ...
                if(flag) {
                    if (typeof (history.pushState) != "undefined") {
                        var update_url = '<?php echo base_url('index.php/bms_fin_coa/coa_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;
        ...
        $('#property_id').focus()
        var str = '<tr><td class="hidden-xs text-center" colspan="7">No Record Found</td>';
        str += '<td class="visible-xs text-center" colspan="4">No Record Found</td></tr>';
        $("#content_tbody").html(str);
    }
