              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add COA Sub Account" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_chart_of_accounts/coa_sub_acc_form');?>';
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_chart_of_accounts/get_coa_sub_acc_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.coa_acc_code+'</td>';
                    str += '<td>'+item.coa_sub_acc_code+'</td>';
                    str += '<td>'+item.coa_acc_name+'</td>';
                    str += '<td>'+item.coa_acc_sub_name+'</td>';
                    str += '<td>'+item.coa_acc_type_name+'</td>';
                    str += '<td>'+item.coa_acc_group_name+'</td>';
                                    
                    str += '<td style="text-align: center;">';
                    str += '<a href="<?php echo base_url('index.php/bms_chart_of_accounts/coa_sub_acc_form');?>/'+item.coa_acc_sub_name_id+'" title="Edit"><i class="fa fa-edit"></i></a>';
                    str += '</td>';
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_chart_of_accounts/charge_codes_list');?>/'+offset+'/'+rows+'?search_txt='+search_txt;
