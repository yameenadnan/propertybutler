            ...
            <div class="box-body">
                <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary" onclick="window.location.href='<?php echo base_url('index.php/bms_fin_accounting/create_bank_trans_old');?>'" value="Create bank transaction (Old)" type="button">
                    </div>
...
<script>
    ...
    function loadContent (offset,rows,flag) {
        ...
        $.ajax({
            type:"post",
            async: true,
            url: '<?php echo base_url('index.php/bms_fin_accounting/get_bank_trans_old_list');?>',
            ...
            success: function(data) {
                ...
                if(numFound > 0) {
                    $.each(data.records,function (i, item) {
                        ...
                        str += '<td>'+item.reference1+'</td>';
                        str += '<td>'+item.trans_date+'</td>';
                        str += '<td>'+item.pmode_name+'</td>';
                        str += '<td>'+item.reference2+'</td>';
                        str += '<td>'+item.debit+'</td>';
                        str += '<td>'+item.credit+'</td>';
                        str += '<td><a href="<?php echo base_url('index.php/bms_fin_accounting/create_bank_trans_old');?>/'+item.bank_trans_old_id+'" title="Edit"><i class="fa fa-edit"></i></a>&ensp;&ensp;';
                        str += '<a href="javascript:;" data-id="' + item.bank_trans_old_id + '" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                        str += '</td>';
                        str += '</tr>';
                    });
                ...
                if(flag) {
                    if (typeof (history.pushState) != "undefined") {
                        var update_url = '<?php echo base_url('index.php/bms_fin_accounting/bank_trans_old_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;
        ...
        function remove_item (id){
            if(confirm('You cannot undo this action. Are you sure want to delete?')) {
                $.ajax({
                    type:"post",
                    async: true,
                    url: '<?php echo base_url('index.php/bms_fin_accounting/delete_bank_trans_old');?>',
                    data: {'bank_trans_old_id':id},
