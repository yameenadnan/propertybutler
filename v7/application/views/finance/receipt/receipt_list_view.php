        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+(item.payment_mode == 0 ? 'Opening Credit' : payment_mode[item.payment_mode])+'</td>';
                    str += '<td>'+item.unit_no+'</td>';                    
                    str += '<td width="20%">'+item.owner_name+'</td>';
                    str += '<td>'+(item.receipt_date != '' ? formatDate(item.receipt_date) : '')+'</td>';
                    //str += '<td>'+(item.bill_due_date != '' ? formatDate(item.bill_due_date) : '')+'</td>';
                    str += '<td>'+(parseFloat(item.paid_amount) > 0 ? item.paid_amount : item.opening_credit)+'</td>';  
                    ...
                    if('<?php echo in_array($_SESSION['bms']['designation_id'],$this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1')  { 
                        ...
                        str += '<a href="javascript:;" data-id="'+item.receipt_id+'" data-deposit-id="'+item.depo_receive_id+'" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                    }
                    str += '</td>';
                    str += '</tr>';
                });
