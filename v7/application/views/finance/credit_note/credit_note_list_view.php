              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-4 col-xs-5">                        
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search Credit Note">
                    </div>
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Credit Note" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_fin_credit_note/add_credit_note');?>?property_id='+$("#property_id").val();
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_fin_credit_note/get_credit_notes_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td><a href="<?php echo base_url().'index.php/bms_fin_credit_note/credit_note_details/';?>'+item.credit_note_id+'">'+item.credit_note_no+'</a></td>';
                    ...
                    str += '<a href="javascript:;" onclick="printDiv(\'<?php echo base_url('index.php/bms_fin_credit_note/credit_note_details');?>/'+item.credit_note_id+'/download\')" title="Download"><i class="fa fa-download"></i></a>&ensp;';
                    str += '<a href="javascript:;" onclick="printDiv(\'<?php echo base_url('index.php/bms_fin_credit_note/credit_note_details');?>/'+item.credit_note_id+'/print\')" title="Print"><i class="fa fa-print"></i></a>&ensp;';
                    if('<?php echo in_array($_SESSION['bms']['designation_id'],$this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1')  { 
                        //str += '<a href="<?php echo base_url('index.php/bms_fin_credit_note/add_credit_note');?>/'+item.credit_note_id+'" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                        //str += '<a href="javascript:;" data-id="'+item.credit_note_id+'" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                    }
                    str += '</td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_fin_credit_note/credit_note_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;
...
function remove_item (id){
    ...
    if(confirm('You cannot undo this action. Are you sure want to delete?')) {
        $.ajax({
            type:"post",
            async: true,
            url: '<?php echo base_url('index.php/bms_fin_credit_note/unset_credit_note');?>',
