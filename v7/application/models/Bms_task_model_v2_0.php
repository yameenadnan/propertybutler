...
Class Bms_task_model_v2_0 extends CI_Model {
    ...
    function get_task ($type = '', $desi_id ='', $staff_id = '', $offset = '', $per_page = '',$property_id='',$task_status='',$task_id='',$sear_txt = '',$sort_by='due_date',$task_update = '') {
        $limit = '';
        if($offset != '' && $per_page != '')
        ...
        if($task_update != '')
            $cond2 .= ' AND a.task_update = \''.$task_update.'\' ';
            
        $sort_by = ($sort_by == 'due_date' ? ' due_date ASC ' :  ' created_date DESC '). ', task_id DESC ';
        
        $num_rows = '';    
        if ($limit != '') {
            $sql = "SELECT task_id,a.property_id,property_name,task_name,due_date,a.created_date,task_status 
                FROM bms_task a 
    ...
    function get_task_with_num_rows ($type = '', $desi_id ='', $staff_id = '', $offset = '', $per_page = '',$property_id='',$task_status='',$task_id='',$sear_txt = '',$sort_by='due_date',$task_update = '') {
        $limit = '';
        if($offset != '' && $per_page != '')
            $limit = ' LIMIT '. $offset .', '.$per_page;
        $cond = $type == '' || $type == 'own' ? '=' : '<>';
    ...
    function set_re_assign ($data,$task_id) {
        $data['updated_by'] = $_SESSION['bms']['staff_id'];
        $data['updated_date'] = date("Y-m-d");
        $this->db->update('bms_task', $data, array('task_id' => $task_id));       
    }
    
    function get_task_with_num_rows_jmb ( $property_id, $task_status='', $offset = '', $per_page = '',$task_id='',$sear_txt = '',$sort_by='due_date', $unit_id = "", $task_update = '') {
        $limit = '';
        if($offset != '' && $per_page != '')
            $limit = ' LIMIT '. $offset .', '.$per_page;
    ...
    function getMinorTaskCount ($property_id,$task_status='O') {
        $date_filed = $task_status == 'O' ? 'created_date' : 'updated_date';
        
        $sql = "SELECT COUNT(task_id) AS cnt
                FROM bms_task a
                WHERE task_status='".$task_status."' AND a.property_id=".$property_id."
    ...
    function getMinorTaskCountForChart ($property_id,$md,$type,$task_status='O') {
        $date_filed = $task_status == 'O' ? 'created_date' : 'updated_date';
    ...
    function getMinorTaskCountForDailyChart ($property_id,$start_date,$end_date,$task_status='O') {
        $date_filed = $task_status == 'O' ? 'created_date' : 'updated_date';
        $date_format = '%d';
        $sql = "SELECT DATE_FORMAT(".$date_filed.",'".$date_format."') AS md, COUNT(1) AS cnt 
                FROM bms_task a
    ...
    function getTaskCount ( $type, $desi_id, $property_id, $due_date_condition ) {
        $cond = $type;
        $sql = "SELECT COUNT(task_id) as cnt 
                FROM bms_task a   
                WHERE a.assign_to " . $cond . " '". $desi_id . "' " .
                " AND a.property_id = '$property_id' 
                AND a.task_status = 'O' 
                AND a.due_date $due_date_condition '" . date('Y-m-d') . "'";
        $query = $this->db->query($sql);
        return $query->row()->cnt;
    }

    function getMinorTaskOpen ($type, $desi_id, $property_id, $due_date_condition, $offset, $rows) {
        $limit = '';
        if($offset != '' && $rows != '')
            $limit = ' LIMIT '. $offset .', '.$rows;
        $cond = $type;

        $order_by = ' ORDER BY a.due_date';

        $sql = "SELECT task_id, task_name, created_date, due_date 
                FROM bms_task a
                WHERE a.assign_to " . $cond . "'" . $desi_id . "'".
                " AND a.property_id = '$property_id' 
                AND a.task_status = 'O' 
                AND a.due_date $due_date_condition '" . date('Y-m-d') . "'" . $order_by;
        $query_num = $this->db->query($sql);
        $query = $this->db->query($sql . $limit);
        $data = $query->result_array();
        return array('numFound'=>$query_num->num_rows(),'records'=>$data);
    }

    function getAnnualRenewalsCount ($property_id) {
        $sql = "SELECT COUNT(annual_renewal_id) AS renewal_count
                FROM bms_annual_renewal a
                WHERE a.property_id=".$property_id."
                AND license_expiry_date between '" . date("Y-m-d") . "' AND '" . date('Y-m-d', strtotime('+3 months')) . "'
                ORDER BY license_expiry_date;";

        $query = $this->db->query($sql);
        return $query->row()->renewal_count;
    }

    function getExpiredAnnualRenewalsCount ($property_id) {
        $sql = "SELECT COUNT(annual_renewal_id) AS renewal_count
                FROM bms_annual_renewal a
                WHERE a.property_id=".$property_id."
                AND license_expiry_date < '" . date('Y-m-d') . "'";

        $query = $this->db->query($sql);
        return $query->row()->renewal_count;
    }

    function getAnnualRenewals ($property_id, $renewal_type, $offset, $rows) {
        ...
        if ( $renewal_type == 'new' )
            $cond = " AND license_expiry_date between '" . date("Y-m-d") . "' AND '" . date('Y-m-d', strtotime('+3 months')) . "'";
        else if ( $renewal_type == 'expired' )
            $cond = " AND license_expiry_date < '" . date('Y-m-d') . "'";

        $order_by = " ORDER BY a.license_expiry_date ";

        $sql = "SELECT annual_renewal_id, item_descrip, serial_no, license_no, license_expiry_date
                FROM bms_annual_renewal a
        ...
        $data = $query->result_array();
        return array('numFound'=>$query_num_rows->num_rows(),'records'=>$data);
    }

    function getServiceScheduleCount ($property_id) {
        $sql = "SELECT COUNT(asset_service_schedule_id) AS asset_service_schedule_count 
        FROM `bms_asset_service_schedule` a
        LEFT JOIN `bms_property_assets` b ON a.`asset_id` = b.`asset_id`
        WHERE asset_service_schedule_id NOT IN
        (
            SELECT service_schedule_id AS asset_service_schedule_id 
            FROM bms_asset_service_details
            WHERE a.`asset_service_schedule_id` = bms_asset_service_details.`service_schedule_id`
        )
        AND b.`property_id` = '$property_id'
        AND a.service_date BETWEEN '" . date("Y-m-d") . "' AND '" . date('Y-m-d', strtotime('+1 months')) . "';";
        $query = $this->db->query($sql);
        return $query->row()->asset_service_schedule_count;
    }

    function getExpiredServiceScheduleCount ($property_id) {
        $sql = "SELECT COUNT(asset_service_schedule_id) AS asset_service_schedule_count 
        FROM `bms_asset_service_schedule` a 
        LEFT JOIN `bms_property_assets` b ON a.`asset_id` = b.`asset_id` 
        WHERE asset_service_schedule_id NOT IN
        ...
        AND b.`property_id` = '$property_id'
        AND a.service_date < '" . date("Y-m-d") . "';";
        $query = $this->db->query($sql);
        return $query->row()->asset_service_schedule_count;
    }

    function getAssetServiceSchedule ($property_id, $type, $offset, $rows) {
        ...
        if ( $type == 'new' )
            $cond = " AND a.service_date between '" . date("Y-m-d") . "' AND '" . date('Y-m-d', strtotime('+1 months')) . "'";
        else if ( $type == 'expired' )
            $cond = " AND a.service_date < '" . date('Y-m-d') . "'";

        $order_by = " ORDER BY a.service_date ";

        $sql = "SELECT a.`asset_id`, a.asset_service_schedule_id, b.asset_name, b.asset_location, a.service_date AS schedule_date
        FROM `bms_asset_service_schedule` a
        LEFT JOIN `bms_property_assets` b ON a.`asset_id` = b.`asset_id`
        WHERE asset_service_schedule_id NOT IN
        (
        ...
        AND b.`property_id` = '$property_id'" . $cond;
