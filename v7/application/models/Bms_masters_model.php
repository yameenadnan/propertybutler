...
Class Bms_masters_model extends CI_Model {
    ...
    function getMyProperties ($staff_id ='') {
        ...
        $sql = "SELECT property_id,property_name,managed_by,property_type,property_abbrev,total_units, bill_due_days, jmb_mc_name, address_1, address_2, phone_no  
    ...
    function getMyPropertiesDeveloper () {
        $email_addr = $_SESSION['bms']['email'];
        $sql = "SELECT b.property_id, b.property_name, b.property_type, b.property_abbrev, b.total_units, b.bill_due_days, b.jmb_mc_name
        FROM bms_property_developer a 
    ...
    function getJmbProperties ($property_id) {  
        ...
        $sql = "SELECT property_id,property_name,managed_by,property_type,total_units 
    ...
    function getPropertyInfo ($property_id) {
        $sql = "SELECT property_id,property_name,property_type,total_units,email_addr,monthly_billing,a.state_id,b.state_name,
                jmb_mc_name, address_1, address_2, phone_no, fax, pin_code, city, managed_by
                FROM bms_property a
    ...
    function getMyPropertiesWithCharg ($staff_id ='') {  
        ...
        $sql = "SELECT property_id,property_name,property_type,total_units,
                calcul_base,sinking_fund,sc_charge,insurance_prem,quit_rent
                FROM bms_property WHERE property_status=1 AND property_id IN 
                (SELECT property_id FROM bms_staff_property WHERE staff_id=".$staff_id.") ORDER BY property_name ASC";
        $query = $this->db->query($sql);
    ...
    function getUnit ($property_id,$block_id = 0,$unit_id = '') {
        $condi = '';
        if($block_id != 0)
            $condi = " AND block_id = '".$block_id."'";
        if($unit_id != 0 && $unit_id != '' )
            $condi .= " AND unit_id = '".$unit_id."'";
        $sql = "SELECT unit_id,unit_no,unit_status,floor_no,owner_name,gender,email_addr,contact_1,is_defaulter,block_id, valid_email  
    ...
    function getStaffNamesByProperty ($property_id = '',$staff_id = '') {
        ...
        $sql = "SELECT staff_id,first_name,last_name  
                    ...
                    AND staff_id NOT IN (1312,1335,1336,1337,1341,1342,1443,1060,1039,1038,1218,1229,1273,1521,1522,1486,1074,1288,1418,1363,1219,1213,1580,1582,1587)
                    AND staff_id IN(
    ...
    function getPropertyStaffs ($property_id,$exclude_desi_ids='') {
        ...
        $sql = "SELECT staff_id,first_name,last_name,mobile_no,desi_name
                ...
                ".$condi."
                AND staff_id NOT IN (1312,1335,1336,1337,1341,1342,1363,1443,1580,1582,1587)
                AND staff_id IN(
                SELECT staff_id from bms_staff_property WHERE property_id =".$property_id.")";
    ...
    function getTiers ($property_id) {
        $query = $this->db->select('tier_id,property_id,tier_name,tier_value')->order_by('tier_name','ASC')->get_where('bms_property_tier',array('property_id'=>$property_id));
        return $query->result_array();
    }

    function getAcbs ($property_id) {
        $query = $this->db->select('acb_id,property_id,acb_remin_name,acb_remin_days')->order_by('acb_remin_name','ASC')->get_where('bms_property_acb',array('property_id'=>$property_id));
        return $query->result_array();
    }

    function get_invalid_email_count ($property_id) {

        $sql = "SELECT COUNT(a.valid_email) AS invalid_email_count 
        ...
        return $query->row()->invalid_email_count;

    }

    function update_unit_set_invalid_email ($data,$unit_id) {
        $this->db->update('bms_property_units', $data, array('unit_id' => $unit_id));
    }

    function auth_dev ($username,$pass) {
        $username = $this->db->escape_str($username);
        $pass = md5($this->db->escape_str($pass));
        $sql = "SELECT a.property_dev_id, c.property_id, c.property_name, a.email_addr, forgot_pass_key, designation 
                FROM bms_property_developer a, bms_property c                
                WHERE c.property_id=a.property_id AND c.property_status=1 
                AND a.email_addr=? AND a.password=?";
        $query = $this->db->query($sql,array($username,$pass));
        return $query->result_array();
    }

    function getDeveloperCredentials ($property_id) {
        $sql = "SELECT a.property_dev_id ,a.email_addr 
                FROM bms_property_developer a
    ...
    function update_pass_dev ($username,$pass) {
        $username = $this->db->escape_str($username);
        $pass = $this->db->escape_str($pass);
        $sql = "UPDATE bms_property_units SET password=? WHERE status=1 AND email_addr=?";
        return $query = $this->db->query($sql,array($pass,$username));
    ...
    function getMyPropertiesForDefects ($staff_id ='') {
        $staff_id = $staff_id == '' ? $_SESSION['bms']['staff_id'] : $staff_id;
        $sql = "SELECT property_id,property_name,property_type,property_abbrev,total_units, bill_due_days, jmb_mc_name, address_1, address_2, phone_no  
                FROM bms_property 
                WHERE property_status=1 
                AND property_id IN 
                (SELECT property_id FROM bms_staff_property WHERE staff_id=".$staff_id.")
                AND property_under = 3 
                ORDER BY property_name ASC";
        $query = $this->db->query($sql);
        //echo $this->db->last_query();
        return $query->result_array();
    ...
    function getDevProperties ($property_id) {
        $sql = "SELECT property_id,property_name,property_type,property_abbrev,total_units, bill_due_days, jmb_mc_name, address_1, address_2, phone_no  
                FROM bms_property WHERE property_status=1 AND property_id = '$property_id'";
    ...
    function checkDeveloperEmailAddrExists ($property_dev_id, $email_addr) {

        $cond = '';
        $cond = !empty ( $property_dev_id ) ? ' AND property_dev_id != ' . $property_dev_id : '';

        $sql = "SELECT email_addr FROM bms_property_developer WHERE email_addr = '$email_addr'" . $cond;
        $query = $this->db->query($sql);
        if ( !empty ( $query->row()->email_addr ) )
            return 1;
        else
            return 0;
    }

    function getPropertyUnderFromPropertyId ( $property_id ) {
        $sql = "SELECT property_under
                FROM bms_property WHERE 1 = 1 AND property_id = '$property_id'";

        $query = $this->db->query($sql);
        return $query->row()->property_under;
    }
}