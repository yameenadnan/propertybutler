...
Class Bms_incident_model extends CI_Model {
    function __construct () { parent::__construct(); }
    
    function incident_insert ($data) {
        $this->db->insert('bms_incident', $data);
        return $insert_id = $this->db->insert_id();
    }

    function incident_update ($data, $incident_id) {
        $this->db->update('bms_incident', $data,array ('incident_id' => $incident_id) );
    }

    function incident_image_name_insert ($data) {
        $this->db->insert('bms_incident_img', $data);
    }

    function get_incident_detals ($incident_id) {
        $sql = "SELECT * FROM bms_incident where incident_id = '$incident_id';" ;
        $query = $this->db->query($sql);
        return $data = $query->result_array();
    }

    function get_image_detals ($incident_id) {
        $sql = "SELECT * FROM bms_incident_img WHERE incident_id = '$incident_id';" ;
        $query = $this->db->query($sql);
        return $data = $query->result_array();
    }
    ...
    function get_all_incident ($staff_id='', $property_id='', $offset = '', $per_page = '') {
        $limit = '';
        if($offset != '' && $per_page != '')
        ...
        $sort_by = 'incident_id DESC ';
        
        $num_rows = '';    
        if ($limit != '') {
            $sql = "SELECT a.incident_id,a.property_id,a.details, a.status, a.incident_date, a.incident_time, a.created_date 
                FROM bms_incident a 
                LEFT JOIN bms_property b ON b.property_id = a.property_id   
                WHERE a.property_id IN (SELECT property_id FROM bms_staff_property WHERE staff_id=".$staff_id. $cond2 .") " ;
            $query = $this->db->query($sql);
        ...
        $sql = "SELECT a.incident_id,a.property_id,a.details, a.status, a.incident_date, a.incident_time, a.created_date 
                FROM bms_incident a 
                LEFT JOIN bms_property b ON b.property_id = a.property_id   
                WHERE a.property_id IN (SELECT property_id FROM bms_staff_property WHERE staff_id=".$staff_id. $cond2 .")  ORDER BY  ".$sort_by . $limit;
