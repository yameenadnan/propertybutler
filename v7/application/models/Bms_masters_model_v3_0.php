...
Class Bms_masters_model_v3_0 extends CI_Model {
    ...
    function getJmbProperties ($property_id) {
        ...
        $sql = "SELECT property_id,property_name,property_type,total_units,account_status,vms_status,vms_access,
                property_under,jmb_mc_name,address_1,address_2,city,pin_code,b.state_name,phone_no,fax,email_addr,
                mob_app_issue,mob_app_billing,mob_app_defect,mob_app_fasc_book,mob_app_pro_doc,
    ...
    function getAvailbleUnits ($email) {

        $sql = "SELECT a.unit_id,a.unit_no,a.property_id,a.contact_1,p.property_name,property_type,account_status,vms_status,vms_access,
                property_under,p.jmb_mc_name,p.address_1,p.address_2,p.city,p.pin_code,b.state_name,p.phone_no,p.fax,p.email_addr,
                mob_app_issue,mob_app_billing,mob_app_defect,mob_app_fasc_book,mob_app_pro_doc,
                mob_app_daily_rpt,mob_app_survey_form,mob_app_visit_list,mob_app_prebook,mob_app_freq_visit,
                mob_app_note_to_guard,mob_app_panic_alert,c.jmb_status
                FROM bms_property_units a
                LEFT JOIN  bms_property p ON p.property_id = a.property_id
                LEFT JOIN bms_state b ON b.state_id=p.state_id
                LEFT JOIN bms_jmb_mc c ON c.unit_id=a.unit_id
                WHERE p.property_status=1 AND a.email_addr= '".$email."'
                ORDER BY property_name ASC, a.unit_no ASC";
        $query = $this->db->query($sql);
        //echo $this->db->last_query();
        return $query->result_array();
    }
    ...
    function getResiNoticeBoard ($property_id) {
        $sql = "SELECT resident_notice_id,property_id,notice_title,start_date,end_date,message,attachment_name
                ...
                ORDER BY resident_notice_id DESC";
        $query = $this->db->query($sql);
...
/*        $sql = "SELECT *
    ...
    function getPaymentChargeDetail($propertyID) {
      $sql = "SELECT pymt_gateway_url, payment_merchant_id,payment_merchant_key_index, payment_merchant_key,
              payment_fpx, payment_cc_card,payment_bear_by, jmb_mc_name
              FROM bms_property WHERE property_id=".$propertyID;
      $query = $this->db->query($sql);
      return $query->result_array();
    }
    ...
    function insertRefNo($data){
      return $this->db->insert('bms_direct_pymt', $data);;
    }

    // function submitDirectPayment($refNo, $data) {
    //   $this->db->where('Reference_Number', $refNo);
    //   $this->db->update('bms_direct_pymt', $data);
    //
    //   $sql = "SELECT * FROM bms_direct_pymt WHERE Reference_Number='".$refNo."'";
    //   $query = $this->db->query($sql);
    ...
    //   $data3 = $query3->result_array();
    //
    //   $sql6 = "SELECT block_id FROM bms_property_units WHERE unit_id=".$unitID;
    //   $query6 = $this->db->query($sql6);
    //   $data6 = $query6->result_array();
    //   $blockID = $data6[0]['block_id'];
    //
    //   $propertyAbbr = $data3[0]['property_abbrev'];
    //   $receipt_no_format = ($propertyAbbr != '' ? $propertyAbbr : 'XX'). '/'.date('y', strtotime($responseDatetime)).'/'.date('m', strtotime($responseDatetime)).'/';
    //
    //   $sql4 = "SELECT receipt_no FROM bms_fin_receipt WHERE receipt_no LIKE '".$receipt_no_format."%' ORDER BY receipt_id DESC LIMIT 1";
    //   $query4 = $this->db->query($sql4);
    //   $data4 = $query4->row_array();
    ...
    //   $insertReceipt['payment_mode'] = 4;
    //   $insertReceipt['bank_id'] = $bankID;
    //   $insertReceipt['bank'] = $data['Bank_Code'];
    //   $insertReceipt['cheq_card_txn_no'] = $data['Transaction_ID'];
    //   $insertReceipt['cheq_txn_online_date'] = date('Y-m-d', strtotime($data['Response_Datetime']));
    //   $insertReceipt['online_r_card_type'] = $data['Payment_ID'];
    //   $insertReceipt['remarks'] = $data['Transaction_Description'];
    //   $insertReceipt['paid_amount'] = $data['Amount'];
    //   $insertReceipt['created_date'] = date('Y-m-d h:i:s');
    //
    //   $this->db->insert('bms_fin_receipt', $insertReceipt);
    //   $receiptID = $this->db->insert_id();
    //
    //   $insertReceiptItem['receipt_id'] = $receiptID;
    //   $insertReceiptItem['item_descrip'] = $data['Transaction_Description'];
    //   $insertReceiptItem['item_amount'] = $data['Amount'];
    //   $insertReceiptItem['paid_amount'] = $data['Amount'];
    //
    //   $this->db->insert('bms_fin_receipt_items', $insertReceiptItem);
    //
    //   return 1;
    ...
    function getPrebookVisitorList($propertyID, $unitNo) {
        ...
        FROM bms_vms_prebook_visitors a
        WHERE a.flag IN (1, 2) AND a.property_id = ".$propertyID." AND a.unit_id = ".$unitNo . " AND a.booking_date between '$yesterday' AND '$today' ";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    function getFacilityDetail ($facility_id) {
        $sql = "SELECT property_id, facility_name, start_time, end_time, booking_slot, number_of_slots, disclaimer, picture
    ...
    function getSlotOccupied ($facility_id, $booking_date) {
    ...
    function insertBookingSlot ($data) {
        $data['created_by'] = '??????';
        $data['created_date'] = date('Y-m-d');;

        return $this->db->insert('bms_property_facility_booking', $data);
    }

    function getBookingSlotListByFacility ( $unit_id ) {
        $sql = "SELECT b.facility_id, b.facility_name, a.facility_booking_id, a.booking_date, a.booking_slot, a.booking_status
              ...
              ORDER BY a.facility_id, a.booking_status";

        $query = $this->db->query($sql);
        return $query->result_array();
    }

    function deleteFacilityBookingSlot ($facility_booking_id) {
        $this->db->where('facility_booking_id', $facility_booking_id);
        return $this->db->delete('bms_property_facility_booking');
    }

    function getFacilitiesByProperty ($property_id) {
    ...
    function getUnitDefaulterStatus ($property_id, $unit_id) {
        $sql = "SELECT is_defaulter
              FROM bms_property_units
              WHERE property_id = '$property_id' " .
            " AND unit_id = '$unit_id'" ;
        $query = $this->db->query($sql);
    ...
    function getDirectPaymentList ($propertyID, $unitID, $rows) {
        $sql = "SELECT a.Payment_ID, a.Transaction_ID, a.Reference_Number, a.Currency, a.Amount, a.Settlement_Amount, a.Transaction_Description, a.Error_Description, a.Response_Datetime, b.property_name,b.jmb_mc_name,b.address_1,b.address_2,b.pin_code,c.block_name,d.state_name,e.country_name,f.unit_no,f.owner_name
            FROM bms_direct_pymt a
            LEFT JOIN bms_property b ON b.property_id = a.property_id
            LEFT JOIN bms_state d ON d.state_id = b.state_id
            LEFT JOIN bms_countries e ON e.country_id = b.country_id
            LEFT JOIN bms_property_units f ON f.unit_id = a.unit_id
            LEFT JOIN bms_property_block c ON c.block_id = f.block_id
            WHERE a.property_id=".$propertyID." AND a.unit_id=".$unitID." AND a.Response_Code = '00' ORDER BY a.Response_Datetime DESC LIMIT ".$rows;
        $query = $this->db->query($sql);
        return $query->result_array();
    }
}
